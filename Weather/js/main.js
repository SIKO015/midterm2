...
async function find(city) {
  let response = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=67deb1cf48e949ebb65115940232302&q=${city}&days=5`,
    { method: "GET" }
  );
  if (response.ok && response.status !== 400) {
    ...
    getDayFour(data.forecast.forecastday);
    getDayFive(data.forecast.forecastday);
    console.log(data);
  }
}
...
function getDayFour() {
  document.querySelector('.day-four .top-info').innerText = "Sunday";
  document.querySelector('.day-four .middle-info').innerHTML = `
    <p>24.5°C</p>
    <p>16.8°C</p>
    <p>Sunny</p>
  `;
...
function getDayFive() {
  document.querySelector('.day-five .top-info').innerText = "Monday";
  document.querySelector('.day-five .middle-info').innerHTML = `
    <p>23.3°C</p>
    <p>15.9°C</p>
    <p>Partly Cloudy</p>
  `;
